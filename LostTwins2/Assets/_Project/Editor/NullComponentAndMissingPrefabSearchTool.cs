...
public class NullComponentAndMissingPrefabSearchTool : MonoBehaviour
{
    static List<GameObject> missingPrefabs = new List<GameObject>();

    [MenuItem("We.R.Play/Log Missing Prefabs And Components")]
    public static void Search()
    {
        ...
        GameObject[] missingObjectsArray = missingPrefabs.ToArray();
        Selection.objects = missingObjectsArray;
    ...
    private static void Traverse(Transform transform, string path = "")
    {
        ...
        for (int c = 0; c < transform.childCount; c++)
        {
            ...
            if (pt == PrefabAssetType.MissingAsset)
            {
                ...
                missingPrefabs.Add(t.gameObject);
            }
            else
            {
                Traverse(t, thisPath);
